    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-gradient-to-br from-yellow-500/20 to-orange-500/20 border border-yellow-500/30 rounded-xl p-4">
          ...
          <div className={`text-3xl font-bold ${restante > 0 ? 'text-yellow-400' : 'text-emerald-400'}`}>
      ...
      <div className="bg-[#0f1419] border border-white/10 rounded-xl p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-6">
            <div>
              <label className="block text-sm text-gray-400 mb-2">💰 BANCA INICIAL:</label>
              <input
                type="number"
                value={bancaInicial}
                onChange={(e) => setBancaInicial(e.target.value === '' ? '' : Number(e.target.value))}
                className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg px-4 py-2 text-white font-bold w-40"
            ...
            <div>
              ...
              <input
                ...
                className="bg-purple-500/10 border border-purple-500/30 rounded-lg px-4 py-2 text-white font-bold w-40"
          ...
          <div className="flex gap-3 items-center">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-500 hover:shadow-lg hover:shadow-blue-500/50 text-white rounded-lg transition-all font-medium"
            >
            ...
            <button
              onClick={handleReset}
              className="flex items-center gap-2 px-4 py-2 bg-red-600/20 hover:bg-red-600/30 border border-red-500/30 text-red-400 rounded-lg transition-all"
            >
        ...
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
          <p className="text-sm text-blue-200">
            <strong>💡 Dica:</strong> Use a coluna <strong className="text-emerald-400">DEPÓSITO</strong> quando adicionar dinheiro nas casas e a coluna <strong className="text-red-400">SAQUE</strong> quando retirar dinheiro. A banca do próximo dia será ajustada automaticamente.
          </p>
        </div>
      </div>
      ...
      <div className="mb-4">
        <button
          onClick={addDay}
          className="w-full py-4 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:shadow-lg hover:shadow-emerald-500/50 text-white rounded-xl font-bold text-lg transition-all"
        >
          + Adicionar Dia
        </button>
      </div>
        ...
        {!isLoaded ? (
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p className="text-gray-400">Carregando planilha...</p>
            </div>
          </div>
        ) : (
                ...
                <tr key={row.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <input
                      ...
                      className="w-full bg-blue-500/10 border border-blue-500/30 rounded px-3 py-1.5 text-right text-white focus:outline-none focus:border-blue-500/50 font-mono"
                      placeholder=""
                    />
                    {parseValues(row.casa1) > 0 && (
                      <div className="text-xs text-blue-400 mt-1 font-bold text-right">
                  ...
                  <td className="px-4 py-3">
                    <input
                      ...
                      className="w-full bg-purple-500/10 border border-purple-500/30 rounded px-3 py-1.5 text-right text-white focus:outline-none focus:border-purple-500/50 font-mono"
                      placeholder=""
                    />
                    {parseValues(row.casa2) > 0 && (
                      <div className="text-xs text-purple-400 mt-1 font-bold text-right">
                  ...
                  <td className="px-4 py-3">
                    <input
                      ...
                      className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded px-3 py-1.5 text-right text-white focus:outline-none focus:border-emerald-500/50 font-mono"
                      placeholder=""
                      title="Valor depositado nas casas de apostas"
                    />
                    {parseValues(row.deposito) > 0 && (
                      <div className="text-xs text-emerald-400 mt-1 font-bold text-right">
                  ...
                  <td className="px-4 py-3">
                    <input
                      ...
                      className="w-full bg-red-500/10 border border-red-500/30 rounded px-3 py-1.5 text-right text-white focus:outline-none focus:border-red-500/50 font-mono"
                      placeholder=""
                      title="Valor sacado das casas de apostas"
                    />
                    {parseValues(row.saque) > 0 && (
                      <div className="text-xs text-red-400 mt-1 font-bold text-right">
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      ...
                      className="text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded p-1 transition-all"
                      title="Deletar dia"
                    >
                      ✕
                    </button>
      ...
      {showFinishModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#0f1419] border border-white/20 rounded-2xl p-6 max-w-md w-full">
            <h3 className="text-2xl font-bold text-white mb-4">📅 Finalizar Mês</h3>
            
            <div className="bg-gradient-to-br from-emerald-500/10 to-cyan-500/10 border border-emerald-500/30 rounded-lg p-4 mb-4">
              <div className="text-sm text-gray-400 mb-2">Resumo do Mês:</div>
            ...
            <div className="mb-4">
              <label className="block text-sm text-gray-400 mb-2">Nome do Mês:</label>
              <input
                type="text"
                value={monthName}
            ...
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 mb-4">
              <p className="text-sm text-blue-200">
                <strong>💡 Atenção:</strong> Ao finalizar, este mês será arquivado e uma nova planilha será iniciada com a banca final deste mês.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2 bg-gray-600/20 hover:bg-gray-600/30 border border-gray-500/30 text-gray-300 rounded-lg transition-all"
              >
                Cancelar
              </button>
              <button
                onClick={confirmFinishMonth}
                className="flex-1 px-4 py-2 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:shadow-lg hover:shadow-emerald-500/50 text-white rounded-lg font-bold transition-all"
              >
                Confirmar
              </button>
            </div>
